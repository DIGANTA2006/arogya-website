...
import { isOnlineAppointmentType } from "@/lib/consultation-flow";
...
export async function PATCH(request: Request) {
  ...
  if (!body.id || !body.status || !validStatuses.includes(body.status)) {
    return NextResponse.json(
      { error: "Invalid appointment status update." },
      { status: 400 }
    );
  }

  const appointments = await getAppointments();
  const currentAppointment = appointments.find((item) => item.id === body.id);

  if (!currentAppointment) {
    return NextResponse.json({ error: "Appointment not found." }, { status: 404 });
  }

  const requiresPaidPayment =
    isOnlineAppointmentType(currentAppointment.appointmentType) &&
    (body.status === "Confirmed" || body.status === "Completed");

  if (requiresPaidPayment) {
    const payment = await getPaymentByAppointmentForPatient({
      appointmentId: currentAppointment.id,
      patientEmail: currentAppointment.email,
    });
    ...
    if (payment?.status !== "paid") {
      return NextResponse.json(
        {
          error:
            "Online appointment cannot be confirmed or completed before payment is marked Paid.",
        },
        { status: 403 }
      );
    }
