...
export async function proxy(request: NextRequest) {
  ...
  if (path.startsWith('/admin/dashboard') || path.startsWith('/admin/appointments')) {
  ...
  if (path.startsWith('/client/dashboard')) {
    ...
    if (!valid) return NextResponse.redirect(new URL('/client/login', request.url))
  }
...
export const config = {
  matcher: ['/admin/dashboard/:path*', '/admin/appointments/:path*', '/client/dashboard/:path*'],
}
